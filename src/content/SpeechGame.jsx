...
const SpeechGame = () => {
  ...
  try {
  console.log("[SpeechGame] Starting audio feature analysis...");
  ...
  console.log("[SpeechGame] Reference MFCC:", refMFCC);
  // Compare
  let distance = euclideanDistance(userMFCC, refMFCC);
  console.log(`[SpeechGame] Euclidean distance between user and reference MFCC: ${distance}`);
  const maxDist = 250;
  ...
  console.log(`[SpeechGame] Distance not finite or too high, setting audioScore to 0.1`);
  } else {
...
// }
  ...
  console.log(`[SpeechGame] Transcript similarity score: ${transcriptSim}`);
  setTranscriptScore(transcriptSim);
  // Combine scores
  const final = (transcriptSim + audioScore) / 2;
  console.log(`[SpeechGame] Final combined score: ${final}`);
  setFinalScore(final);
  setFeedback(
  ...
  setTranscriptScore(null);
  setFinalScore(null);
  console.error("[SpeechGame] Analysis failed:", err);
  await Swal.fire({
  icon: 'error',
  ...
  const extractMFCC = async (audioBuffer) => {
    ...
    console.log(`[SpeechGame] Extracted ${mfccs.length} MFCC frames.`);
    if (mfccs.length === 0) {
      console.log("[SpeechGame] No MFCC frames extracted.");
      return null;
    }
    ...
    console.log("[SpeechGame] Averaged MFCC:", avgMFCC);
    return avgMFCC;
  };
  ...
  const euclideanDistance = (a, b) => {
    if (!a || !b || a.length !== b.length) {
      console.log("[SpeechGame] Invalid MFCC vectors for distance calculation.", a, b);
      return 9999;
    }
    const dist = Math.sqrt(a.reduce((sum, val, i) => sum + Math.pow(val - b[i], 2), 0));
    console.log(`[SpeechGame] Calculated Euclidean distance: ${dist}`);
    return dist;
  };
